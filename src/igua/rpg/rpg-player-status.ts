...
export class RpgPlayerStatus implements RpgStatus.Model {
    ...
    readonly conditions = ((): RpgStatus.Model["conditions"] => {
        ...
        return ({
            ...
            poison: {
                ...
                get max() {
                    return Math.round(
                        100
                            * (1
                                + buffs.getAggregatedBuffs().conditions.poisonMaxIncreaseFactor
                                    * (state.conditions.poison.level + 1) / 100),
                    );
                },
                get rateFactor() {
            ...
            wetness: {
                ...
                get max() {
                    return Math.round(
                        100 * (1 + buffs.getAggregatedBuffs().conditions.wetnessMaxIncreaseFactor / 100),
                    );
                },
            },
            overheat: {
    ...
    readonly guardingDefenses = ((): RpgStatus.Defenses => {
        const self = this;
    ...
    readonly defenses = ((): RpgStatus.Defenses => {
        const buffs = this._buffs;
