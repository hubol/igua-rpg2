...
export function objNetGift(client: IguaClient) {
    ...
    return container(hitboxObj)
        ...
        .mixin(mxnCutscene, function* () {
            ...
            if (!giftItem) {
                yield* show("Gifting item...");
                const transaction = client.offer({ id: "AnnoyIguanas", kind: "potion" });
                yield () => Boolean(transaction.outcome);
                yield* show(transaction.outcome!.accepted ? "Success" : "Someone beat you to it.");
            }
            else {
