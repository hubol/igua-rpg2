...
export function objFigureKeyItem(keyItemId: DataKeyItem.Id) {
    ...
    return container(
        new Graphics().beginFill(props.backgroundTint).drawRoundedRect(0, 0, 32, 32, 6),
        objText.MediumBoldIrregular(props.name, { tint: props.textTint, maxWidth: 32, align: "center" })
            .anchored(0.5, 0.5)
            .at(16, 16),
    );
}
...
function getPlaceholderProperties(keyItemId: DataKeyItem.Id) {
    let seed = keyItemId.length * 698769;
    for (let i = 0; i < keyItemId.length; i++) {
        seed += keyItemId.charCodeAt(i) * 9901237 + 111 - i * 3;
    }
    ...
    return {
        backgroundTint,
        textTint,
        name: DataKeyItem.getById(keyItemId).name
