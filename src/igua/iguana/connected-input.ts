...
export namespace ConnectedInput {
    ...
    export function find<
        TShape extends TypedInput.Any,
        TKind extends TShape["kind"] = TShape["kind"],
        TLeaf = TShape & { kind: TKind },
    >(
        instance: Tree<unknown>,
        kind: TKind,
    ): Leaf<TLeaf>[] {
    ...
    export type Tree<T> = {
        [k in keyof T]: T[k] extends TypedInput.Any ? Leaf<T[k]>
            : T[k] extends Record<string, unknown> ? Tree<T[k]>
            : never;
    };
