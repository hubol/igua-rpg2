 ...
 * @memberof PIXI
 */
export class AsshatText extends Container implements PixiAnchored.Anchorable {
     ...
     * @private
     */
    protected _text!: string | txt.Type;
    ...
    anchored(x_vector: VectorSimple | number, y?: number): this {
    ...
    public updateText(): void {
            ...
            if (!pagesMeshData[baseTextureUid]) {
                ...
                if (!pageMeshData) {
                    ...
                    pageMeshData = {
                        ...
                        total: 0,
                        mesh,
                        // @ts-expect-error Stupid x1
                        vertices: null,
                        // @ts-expect-error Stupid x2
                        uvs: null,
        ...
        for (const i in pagesMeshData) {
            ...
            if (this.anchor.x !== 0 || this.anchor.y !== 0) {
                let vertexCount = 0;
                // Patched here
                const anchorOffsetX = Math.round(this._textWidth * this.anchor.x),
                    anchorOffsetY = Math.round(this._textHeight * this.anchor.y);
    ...
    public get text(): string | txt.Type {
        return this._text;
    }

    public set text(text: string | txt.Type) {
        if (this._text === text) {
            return;
        }
        this._text = text;
